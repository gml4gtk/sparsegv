...
static const char *parse_escape(const char *p, unsigned *val, const char *end, int bits, struct position pos)
{
	...
	case 'x':{
			...
			for (c = 0; p < end; c = (c << 4) + d) {
				...
				p++;
				if (c & mask) {
					warning(pos, "hex escape sequence out of range");
					mask = 0;
				}
			}
			break;
	...
	case '0' ... '7':{
			...
			if ((c & 0400) && bits < 9)
				warning(pos, "octal escape sequence out of range");
			break;
		}
...
void get_char_constant(struct token *token, unsigned long long *val)
{
	...
	p = parse_escape(p, &v, end, type < TOKEN_WIDE_CHAR ? bits_in_char : wchar_ctype->bit_size, token->pos);
	if (p != end)
		warning(token->pos, "multi-character character constant");
	*val = v;
}
