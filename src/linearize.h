...
struct instruction {
	unsigned opcode:7, tainted:1, size:24;
...
static inline int bb_terminated(struct basic_block *bb)
{
	...
	return insn && insn->opcode >= OP_TERMINATOR && insn->opcode <= OP_TERMINATOR_END;
}
...
static inline void replace_bb_in_list(struct basic_block_list **list, struct basic_block *old, struct basic_block *new, int count)
{
...
extern void insert_select(struct basic_block *bb, struct instruction *br, struct instruction *phi, pseudo_t if_true,
			  pseudo_t if_false);
