...
#ifdef PARANOIA
static void verify_seq_sorted(struct ptr_list *l, int n, int (*cmp)(const void *, const void *))
{
	int i = 0;
	const void *a;
...
// of the merged list.
static struct ptr_list *merge_block_seqs(struct ptr_list *b1, int n,
					 struct ptr_list *b2, int m, int (*cmp)(const void *, const void *))
{
	int i1 = 0, i2 = 0;
	...
	while (b1->nr == 0 || cmp(PTR_ENTRY_NOTAG(b1, b1->nr - 1), PTR_ENTRY_NOTAG(b2, 0)) < 0) {
...
void sort_list(struct ptr_list **plist, int (*cmp)(const void *, const void *))
{
	...
	while (1) {
		...
		do {
			...
			newhead = merge_block_seqs(block1, blocks, block2, i, cmp);
#ifdef PARANOIA
