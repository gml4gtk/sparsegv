...
const char *show_instruction(struct instruction *insn)
{
	...
	case OP_PTRTU:
	case OP_PTRCAST:
		buf += sprintf(buf, "%s <- (%d) %s", show_pseudo(insn->target), type_size(insn->orig_type), show_pseudo(insn->src));
		break;
	...
	case OP_SLICE:
		buf +=
		    sprintf(buf, "%s <- (%d) %s, %d", show_pseudo(insn->target), type_size(insn->orig_type), show_pseudo(insn->src),
			    insn->from);
		break;
...
void insert_select(struct basic_block *bb, struct instruction *br, struct instruction *phi_node, pseudo_t if_true,
		   pseudo_t if_false)
{
	pseudo_t target;
...
static int linearize_simple_address(struct entrypoint *ep, struct expression *addr, struct access_data *ad)
{
...
static int linearize_address_gen(struct entrypoint *ep, struct expression *expr, struct access_data *ad)
{
...
static pseudo_t linearize_store_gen(struct entrypoint *ep, pseudo_t value, struct access_data *ad)
{
...
static pseudo_t linearize_bitfield_extract(struct entrypoint *ep, pseudo_t val, struct symbol *ctype, struct symbol *btype)
{
...
static pseudo_t linearize_logical_branch(struct entrypoint *ep, struct expression *expr, struct basic_block *bb_true,
					 struct basic_block *bb_false);

static pseudo_t linearize_cond_branch(struct entrypoint *ep, struct expression *expr, struct basic_block *bb_true,
				      struct basic_block *bb_false);
...
static pseudo_t add_join_conditional(struct entrypoint *ep, struct expression *expr, pseudo_t phi1, pseudo_t phi2)
{
	pseudo_t target;
...
static pseudo_t linearize_short_conditional(struct entrypoint *ep, struct expression *expr,
					    struct expression *cond, struct expression *expr_false)
{
	pseudo_t src1, src2;
	struct basic_block *bb_false;
...
static pseudo_t linearize_conditional(struct entrypoint *ep, struct expression *expr,
				      struct expression *cond, struct expression *expr_true, struct expression *expr_false)
{
	pseudo_t src1, src2;
	pseudo_t phi1, phi2;
...
static void insert_phis(struct basic_block *bb, pseudo_t src, struct symbol *ctype, struct instruction *node)
{
...
static pseudo_t linearize_expression(struct entrypoint *ep, struct expression *expr)
{
	...
	case EXPR_CONDITIONAL:
		...
		return linearize_conditional(ep, expr, expr->conditional, expr->cond_true, expr->cond_false);
