 ...
 */
static int rewrite_branch(struct basic_block *bb, struct basic_block **ptr, struct basic_block *old, struct basic_block *new)
{
...
static int simplify_branch_branch(struct basic_block *bb, struct instruction *br, struct basic_block **target_p, int bb_true)
{
...
static int simplify_one_branch(struct basic_block *bb, struct instruction *br)
{
	if (simplify_phi_branch(bb, br))
		return 1;
	return simplify_branch_branch(bb, br, &br->bb_true, 1) | simplify_branch_branch(bb, br, &br->bb_false, 0);
}
	...
	if (insn->bb && pseudo->type == PSEUDO_SYM) {
		...
		if (sym->bit_size > 0 && (offset < 0 || bit > sym->bit_size)) {
			...
			warning(insn->pos, "invalid access %s '%s' (%d %d)",
				...
				show_ident(sym->ident), offset, bits_to_bytes(sym->bit_size));
			insn->tainted = 1;
		}
	}
}
