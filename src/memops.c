...
static int find_dominating_parents(struct instruction *insn, struct basic_block *bb, struct pseudo_list **dominators, int local)
{
...
{
	return pseudo->type == PSEUDO_SYM && !(pseudo->sym->ctype.modifiers & (MOD_STATIC | MOD_NONLOCAL))
...
static bool try_to_kill_store(struct instruction *insn, struct instruction *dom, int local)
{
